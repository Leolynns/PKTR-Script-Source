...
agent_hit = (ti_on_agent_hit, 0, 0, [], # server: apply extra scripted effects for special weapons, hitting animals, and when overloaded by armor
   [(store_trigger_param_1, ":attacked_agent_id"),
    ...
    (store_trigger_param_3, ":damage_dealt"),
		(try_begin), # leolynn yarali damage.
			(agent_get_player_id, ":attacked_player_id", ":attacked_agent_id"),
			(player_get_slot, ":is_wounded", ":attacked_player_id", slot_player_is_wounded),
			(gt, ":is_wounded", 0),
			(set_trigger_result, 0),
			(agent_get_player_id, ":attacker_player_id", ":attacker_agent_id"),
			(agent_set_animation, ":attacked_agent_id", "anim_sleeping_final"),
			(str_store_string , s32 , "@Saldirdiginiz kisi su an zaten yarali oldugundan hasar veremezsiniz!"),
      (call_script , "script_colorful_message" , ":attacker_player_id" , 0xFFFF33),
		(try_end),
		(try_begin), # leolynn yarali damage 2.
			(agent_get_player_id, ":attacker_player_id", ":attacker_agent_id"),
			(player_get_slot, ":is_wounded", ":attacker_player_id", slot_player_is_wounded),
			(gt, ":is_wounded", 0),
			(set_trigger_result, 0),
			(agent_set_animation, ":attacker_agent_id", "anim_sleeping_final"),
			(str_store_string , s32 , "@Yarali oldugundan dolayi hasar veremezsin!"),
      (call_script , "script_colorful_message" , ":attacker_player_id" , 0xFFFF33),
		(try_end),
   ...
   [(multiplayer_is_server),
    ...
    (try_for_range, ":player_id", "$g_loop_player_id", ":loop_end"), # continue from the last player id checked
      ...
      (try_begin),
        ...
        (multiplayer_send_string_to_player , ":player_id" , server_event_local_chat_shout , "@Artik hareket edebilirsiniz, duruma gore yarali rolu yapiniz."),
        ...
        #(call_script, "script_player_respawn_in_place", ":player_id"),
			...
			(else_try),
				...
				(agent_set_animation, ":agent_id", "anim_sleeping_final"),
      (try_end),   
