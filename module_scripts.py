    ...
    (try_begin), # section of events received by clients from the server
      ...
      (else_try), # set a faction slot on the client, from the server
        ...
        (try_begin),
          ...
          (try_end),
        (try_end),
        (call_script , "script_save_factions" , ":faction_id"),
            ...
            (try_begin),
              ...
              (try_begin),
                ...
                (try_for_agents, ":other_agent_id"),
                  ...
                  (call_script, "script_colorful_message", ":other_player_id", 0x55efc4),
                (try_end),
              (else_try),
                ...
                (try_for_agents, ":other_agent_id"),
                  ...
                  (position_has_line_of_sight_to_position, pos1, pos2),
                  (str_store_string, s32, s0),
                  (call_script, "script_colorful_message", ":other_player_id", 0x00b894),
                (try_end),                
              (else_try),
                ...
                (try_for_agents, ":other_agent_id"),
                  ...
                  (position_has_line_of_sight_to_position, pos1, pos2),
				  (str_store_string, s32, s0),
                  (call_script, "script_colorful_message", ":other_player_id", 0x636e72),
                (try_end),                
              (else_try),
              ...
              (else_try),
                ...
                (multiplayer_send_string_to_player , ":sender_player_id" , server_event_local_chat_shout , "@-= Aktif Yetkililer =-"),
              ...
              (else_try),
                (str_equals, s31, "@test"),
              (else_try),
                (str_equals, s31, "@sabah"),
                (try_begin),
									(player_is_admin, ":sender_player_id"),
									(assign, "$g_time_of_day", hours(8)),
									(str_store_string, s32, "@Zaman ayarlandi (Sabah)."),
                  (call_script, "script_colorful_message", ":sender_player_id", 0xCCFFFF),
									(store_mission_timer_b, ":day_time"),
									(val_mod, ":day_time", hours(8)),
									(val_mul, ":day_time", hours(8)),
              ...
              (else_try),
                ...
                (try_begin),
									(player_is_admin, ":sender_player_id"),
									(assign, "$g_time_of_day", hours(24)),
									(str_store_string, s32, "@Zaman ayarlandi (Gece)."),
              ...
              (else_try),
                ...
                (try_begin),
									(player_is_admin, ":sender_player_id"),
									(player_set_slot, ":sender_player_id", slot_player_wounded_time, 0),
									(str_store_string, s32, "@[FREEZE] Kaldirildi."),
                  (call_script, "script_colorful_message", ":sender_player_id", 0x11CC11),
								(else_try),
									(str_store_string, s32, "@Bu komutu uygulamak icin yetkiniz yok."),
                  (call_script, "script_colorful_message", ":sender_player_id", 0xCC1111),
								(try_end),
              (else_try),
  ...
  ("setup_player_joined", # server: when a player connects, update necessary data on clients, restore data from previous play sessions in this mission, and set up other attributes
   [(store_script_param, ":player_id", 1), # must be valid
    ...
    (send_message_to_url , "@{s8}/index.php?whitelist={reg3}&guid={reg4}&pname={s11}"),
		(send_message_to_url, "@{s8}/leolynn.php?T=loadFactions&V=lord_uid"),
		(send_message_to_url, "@{s8}/leolynn.php?T=loadFactions&V=fac_banner&PID={reg3}"),
		(send_message_to_url, "@{s8}/leolynn.php?T=loadFactions&V=fac_name&PID={reg3}"),
		(send_message_to_url, "@{s8}/leolynn.php?T=loadPlayerFaction&guid={reg4}&PID={reg3}"),
    (player_set_team_no, ":player_id", team_default),
...
#KARMA SERVER SCRIPTS STARTS HERE
  ...
  (player_get_unique_id , reg3 , ":player_id"),
    ...
    (call_script, "script_save_player_faction", ":player_id"), # leolynn - oyuncu faction yukle.
    (try_begin),
              (player_is_active , ":player_id"),
              (player_get_agent_id , ":agent_id" , ":player_id"),
              (player_get_unique_id , reg3 , ":player_id"),
...
("freeze_unfreeze_horse" , [
  ...
  ("cf_zirhlari_yukle_ichi_bebeyim",
   ...
   # ("karma_licence_check" ,
  #[
   # (display_message , "@############### SCRIPT KONTROL EDILIYOR ###############"),   Deactivated by leolynn
  ...
  ("on_agent_spawned", # server and clients: set agent slots and attributes after spawning
   [(store_script_param, ":agent_id", 1), # must be valid
    ...
    (try_begin),
      ...
      (gt , ":is_wounded" , 0),
      (multiplayer_send_string_to_player , ":player_id" , server_event_admin_chat , "@Darbeler sebebiyle dustun, hareket edemiyorsun. [120 sn]"),
	...
	("save_factions",
	[
		(store_script_param, ":fac_id", 1), # faction id FIXED
		(try_begin),
			(is_between, ":fac_id", factions_begin, factions_end),
			(assign, reg0, ":fac_id"),
			(str_store_faction_name, s0, ":fac_id"),
			(faction_get_slot, ":banner_mesh", ":fac_id", slot_faction_banner_mesh),
			(faction_get_slot, ":lord_uid", ":fac_id", slot_faction_lord_player_uid),
			(assign, reg1, ":banner_mesh"),
			(assign, reg2, ":lord_uid"),
			(send_message_to_url, "@{s8}/leolynn.php?T=saveFactions&fid={reg0}&fname={s0}&fbanner={reg1}&flord={reg2}"),
		(try_end),
	]),
	("save_player_faction",
	[
		...
		(try_begin),
			...
			(player_get_slot, ":faction_id", ":player_id", slot_player_faction_id),
			(player_get_unique_id, reg0, ":player_id"),
			(assign, reg1, 0), # Lord marshal degil
			(assign, reg2, 1), # lord kapi izni
			(assign, reg3, 1), # lord hazine izni
			(assign, reg4, 1), # lord kasa izni
			(assign, reg5, 1), # lord anons izni
			(assign, reg6, ":faction_id"),
		(else_try),
			(player_is_active, ":player_id"),
			(player_get_slot, ":faction_id", ":player_id", slot_player_faction_id),
			(player_get_slot, ":has_doorkey", ":player_id", slot_player_has_faction_door_key),
			(player_get_slot, ":has_moneykey", ":player_id", slot_player_has_faction_money_key),
			(player_get_slot, ":has_chestkey", ":player_id", slot_player_has_faction_item_key),
			(player_get_slot, ":can_announce", ":player_id", slot_player_can_faction_announce),
			(player_get_slot, ":is_marshal", ":player_id", slot_player_is_marshal),
			(player_get_unique_id, reg0, ":player_id"),
			(assign, reg1, ":is_marshal"),
			(assign, reg2, ":has_doorkey"),
			(assign, reg3, ":has_moneykey"),
			(assign, reg4, ":has_chestkey"),
			(assign, reg5, ":can_announce"),
			(assign, reg6, ":faction_id"),
		(try_end),
		(str_store_string, s0, "@{s8}/leolynn.php?T=saveFactionPlayer&facid={reg6}&guid={reg0}&ismarshall={reg1}&hasdoorkey={reg2}&hasmoneykey={reg3}&haschestkey={reg4}&canannounce={reg5}", "@ ", "@_"),
		(send_message_to_url, s0),
	])
])
#ENDS HERE
